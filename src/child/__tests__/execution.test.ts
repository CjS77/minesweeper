...
type Script = ExecutorScript | ReviewerScript;
...
function scriptedRunner(
  ...
  const fn: RunSubagentFn = async (opts) => {
    ...
    if (next.role !== opts.role) {
      throw new Error(
        `scriptedRunner: expected ${next.role}, got ${opts.role} (iteration=${opts.iteration ?? "?"})`,
      );
    }
    calls.push({
      role: opts.role as "executor" | "reviewer",
...
function makeStubGit(initial: { headSha: string; mergeBaseSha: string }): StubGit {
  ...
  const stub: StubGit = {
    ...
    commitsAhead: recorder(
      "commitsAhead",
      async (_cwd: string, _base: string) => (head === mergeBaseSha ? 0 : 1),
    ),
...
describe("parseReviewerVerdict", () => {
  ...
  it("prefers 'with minor concerns' over plain Approved", () => {
    expect(parseReviewerVerdict("Verdict: Approved with minor concerns")).toBe(
...
describe("runExecution — clean approval first round", () => {
  it("runs executor once, reviewer once, then squashes, pushes, and opens a PR", async () => {
    ...
    expect(calls.map((c) => c.role)).toEqual(["executor", "reviewer"]);
    ...
    // PR creation got the right base/head and the body references the issue.
    ...
    expect(prArgs?.body).toContain("Fixes #42");
  ...
  it("re-runs the executor with review feedback and finalises after approval", async () => {
    const { result, calls, createPr, emit } = await run({
      responses: [
        ...
        { role: "executor", text: "fixed it", newHeadSha: "AFTER2" },
        { role: "reviewer", text: "# Review\n## Findings\nNone.\n\nVerdict: Approved\n" },
      ],
    });

    expect(calls.map((c) => c.role)).toEqual(["executor", "reviewer", "executor", "reviewer"]);
    expect(calls.map((c) => c.iteration)).toEqual([1, 1, 2, 2]);
...
describe("runExecution — resumption", () => {
  it("skips the executor when entering with status=Reviewing", async () => {
    ...
    expect(calls.map((c) => c.role)).toEqual(["reviewer"]);
    expect(createPr).toHaveBeenCalledTimes(1);
  });
